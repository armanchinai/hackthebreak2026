# HackTheBreak 2026

Welcome to **HackTheBreak 2026**! 🎉  
We're excited to have you join us for a weekend of building, learning, and collaborating with fellow BCIT students at HackTheBreak (HTB).

HackTheBreak is a student-run hackathon where participants come together to build innovative projects, learn new technologies, and compete for prizes.

This event would not be possible without the support of our sponsors, volunteers, and most importantly, our amazing hackers. Whether you're a seasoned hackathon veteran or attending your first event, we can't wait to see what you create at HTB 2026!

Read on for important information about the event, including the schedule, location, team formation, and more.

---

## Docs Structure

- **[Event Details](./event-details.md)**: Key details about the event, schedule, location, and more.
- **[Hacking Resources](./hacking-resources.md)**: A curated list of resources available to hackers during the event, including mentor details and workshop information.
- **[Competition Details](./competition-details.md)**: Information about prizes, judging criteria, and submission guidelines.
- **[Rules & Code of Conduct](./rules.md)**: Important rules and guidelines to ensure a safe and inclusive environment for all participants.

---

## Quick Links

These are the important links you'll need throughout the event:

- **[Discord](https://discord.gg/BdTWfUJRKw)**
- **Devpost (Project Submissions): TBA**
- **[HackTheBreak Website](https://hackthebreak.com)**

!!! tip "Join the Discord"
    All announcements, mentor help, and team formation will happen on **Discord**. Make sure you join as soon as possible so you don't miss important updates.

---

Continue onto the [Event Details](./event-details.md) to learn more about the schedule, location, and what to expect at HackTheBreak 2026!